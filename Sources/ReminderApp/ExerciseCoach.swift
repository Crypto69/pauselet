...
/// a finished exercise shows on all of them. The session itself is the pure
/// `ExerciseSession` from the core; this class only feeds it the clock and
...
final class ExerciseCoach: ObservableObject {
    ...
    /// cancelled, so Space starts the obvious next thing.
    ...
    private func tick() {
        ...
        // Exercises whose last set has run are done: ticked on every display,
        // with the finishing chime, and the suggestion moves on. A cancelled
        // one that was jumped over is left as cancelled.
