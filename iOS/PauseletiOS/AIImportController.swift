...
/// available.
///
/// The iOS twin of `Sources/ReminderApp/AIImportController.swift`. It is
/// duplicated rather than shared because it is app-layer glue on both sides;
/// everything it actually does — the keychain store, the interpreter, the
/// model list — lives in `ReminderAI` and is genuinely shared, including the
/// `aiImportKeyAccount` name, so a key stored by one build is found by the
