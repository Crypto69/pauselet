...
/// A protocol so the coach can be driven without a synthesizer in tests.
@MainActor
...
/// The one speech synthesizer in the app, so the Settings Test button uses the
/// same voice path the coach does.
///
/// The iOS twin of `Sources/ReminderApp/SpeechCoach.swift`. The synthesizer
/// half is identical — AVFoundation is the same framework on both platforms —
/// and the difference is the audio session: on iOS nothing is audible unless a
/// category has been activated, and the coach must keep talking while other
/// audio plays and while the screen is locked.
@MainActor
final class SpeechCoach: NSObject, ObservableObject, SpeechCoaching {
    ...
    /// Cues must be audible over music and podcasts, and must not be silenced
    /// by the Ring/Silent switch — someone mid-exercise is listening for the
    /// next rep, not watching the screen. `.duckOthers` drops the other audio
    /// for the length of a cue rather than stopping it, so a playlist survives
    /// a set; `.playback` is what lets it keep speaking with the screen locked
    /// (there is no `.mixWithOthers` here — that would forbid ducking).
    ///
    /// Set on every utterance rather than once at launch: `Sounds` moves the
    /// shared session to `.ambient` for its chimes, and the last writer wins.
    ...
    /// Hands the audio route back when the coach is finished, so ducked music
    /// returns to full volume instead of staying quiet after the last cue.
...
///
/// Identical in intent to the Mac's catalogue; the one platform difference is
/// that iOS gates personal voices behind an authorisation request, so they are
/// filtered out here too rather than appearing in the picker and then failing.
enum VoiceCatalog {
    ...
    static func installedVoices() -> [Voice] {
        let english = AVSpeechSynthesisVoice.speechVoices().compactMap { voice -> Voice? in
            if voice.voiceTraits.contains(.isPersonalVoice) { return nil }
